...
if row
    nSmp = size(fea,1);
    feaNorm = max(1e-14,full(sum(fea.^2,2)));       % sum(A,2)得到一列向量；   feaNorm为一列向量
    fea = spdiags(feaNorm.^-.5,0,nSmp,nSmp)*fea;
else
    nSmp = size(fea,2);
    feaNorm = max(1e-14,full(sum(fea.^2,1))');      % sum(A,1)得到一行向量；   feaNorm为一列向量
    fea = fea*spdiags(feaNorm.^-.5,0,nSmp,nSmp);    % A=spdiags(B，D ,m, n),产生一个m×n稀疏矩阵A，其元素是B中的列元素放在由D指定的对角线位置上
end            
return;
...
if row
    ...
    if issparse(fea)
        ...
        feaNorm = sum(fea.^2,2).^.5;            % sum（A,2）得到一列向量
    ...
    if issparse(fea)
        ...
        feaNorm = sum(fea.^2,1).^.5;            % sum（A,1）得到一行向量
        fea = fea./feaNorm(ones(1,mFea),:);
    end
end          